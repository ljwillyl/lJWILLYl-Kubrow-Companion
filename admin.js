...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadEvidence(kubrowId){activeEvidence=null;$("editEvidence").textContent="Loading evidence…";const{data,error}=await sb.from("kubrow_scan_evidence").select("*").eq("kubrow_id",kubrowId).order("created_at",{ascending:false}).limit(1).maybeSingle();if(error){$("editEvidence").textContent=error.message;return}if(!data){$("editEvidence").textContent="Legacy record: no V3.1 scan evidence.";return}activeEvidence=data;$("editEvidence").innerHTML=`<div><b>Overall:</b> ${data.overall_confidence??"—"}%</div><div><b>Primary:</b> ${data.primary_confidence??"—"}% · RGB ${(data.primary_rgb||[]).join(", ")}</div><div><b>Secondary:</b> ${data.secondary_confidence??"—"}% · RGB ${(data.secondary_rgb||[]).join(", ")}</div><div><b>Tertiary:</b> ${data.tertiary_confidence??"—"}% · RGB ${(data.tertiary_rgb||[]).join(", ")}</div><div><b>Sample radius:</b> ${data.sample_radius??"—"} px · <b>Scanner:</b> ${esc(data.scanner_version)}</div>`}
